...
public class CommonController extends ControllerBase{
	...
	public ModelAndView toCaseLiAnPage(HttpServletRequest request) {
		...
		String flowdefid = request.getParameter("flowdefid");
		String userid =request.getParameter("valueField");
		String casetype = "";
		String scearch = "";
...
//		SystemUser user = getLoginUser(request);
	...
	/**
	 * 这个方法 根据罪犯的身份展示对应的承办单（无期死缓承办单，重要罪犯承办单）
	 * @author  2017-10-27
	 */
	@RequestMapping("/redirectCBD.json")
	@ResponseBody
	public Object redirectCBD(HttpServletRequest request){
		Map<String,Object> paramap = this.parseParamMap(request);
		String flowdraftids = paramap.get("flowdraftid") ==null?"":paramap.get("flowdraftid").toString();
		if(StringNumberUtil.notEmpty(flowdraftids)){
			if(!flowdraftids.endsWith("'")){
		...
		Map<String,Object> objectMap=flowBaseService.distinguishFinishAtSJCZ(paramap);
		if(objectMap.get("endinSJCZ").toString().length()>0){
		return	new ModelAndView("forward:/baseDocOperate.page?1=1&tempid=JXJS_JXSH_1&flowdefid=other_zfjyjxjssp&solutionid=702724&optype=edit");
		}else if(objectMap.get("keepGoing").toString().length()>0){
		return	new ModelAndView("forward:/baseDocOperate.page?1=1&tempid=JXJS_JXSH&flowdefid=other_zfjyjxjssp&solutionid=702724&optype=edit");
